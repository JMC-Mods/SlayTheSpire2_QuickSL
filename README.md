...
- 默认热键为 `F5`
- 支持单人局快速重新载入当前局存档
- 支持多人局由主机发起同步快速 SL，单人和多人复用同一个热键
- 多人局可配置是否在执行前弹窗询问所有客机
- 关闭询问后，主机会直接通知客机同步执行快速 SL

### 多人快速 SL 流程
1. 主机按下快速 SL 热键。
2. 如果启用了“多人 SL 前询问客机”，所有已连接客机会弹出确认窗口。
3. 所有客机同意后，主机把当前多人存档作为本次加载的权威数据发送给客机，并同步执行快速 SL。
4. 任意客机拒绝、超时，或当前状态不适合加载时，本次快速 SL 会取消。

> 客机按下热键不会发起多人 SL；多人局只有主机可以发起。
